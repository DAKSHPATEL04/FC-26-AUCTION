...
router.get("/", authenticateJWT as any, async (req: AuthRequest, res: Response) => {
  try {
    ...
    let watchlist = await Watchlist.findOne({ ownerId, seasonId: season._id }).populate("players");
    if (!watchlist) {
...
router.post("/toggle", authenticateJWT as any, async (req: AuthRequest, res: Response) => {
  try {
    const ownerId = req.user?.id;
    ...
    let watchlist = await Watchlist.findOne({ ownerId, seasonId: season._id });
    if (!watchlist) {
      watchlist = await Watchlist.create({
        ownerId,
        seasonId: season._id,
