...
router.get("/", async (req, res) => {
    try {
        const { search, position, positionGroup, ratingMin, ratingMax, nation, club, league, status, sortBy = "rating", sortOrder = "desc", page = "1", limit = "24", } = req.query;
        const filter = {};
        if (search) {
        ...
        if (ratingMin || ratingMax) {
            ...
            if (ratingMax)
                filter.rating.$lte = Number(ratingMax);
        }
        if (nation)
        ...
        const [players, total] = await Promise.all([
            Player_js_1.Player.find(filter)
                .sort({ [sortField]: sortDirection })
                .skip(skip)
                .limit(limitNum)
                .lean(),
            Player_js_1.Player.countDocuments(filter),
        ]);
        res.json({
            players,
            total,
...
router.get("/filters", async (req, res) => {
    try {
        const [nations, clubs, leagues] = await Promise.all([
            Player_js_1.Player.distinct("nation"),
            Player_js_1.Player.distinct("club"),
            Player_js_1.Player.distinct("league"),
        ]);
        res.json({
...
router.get("/:id", async (req, res) => {
    try {
        const player = await Player_js_1.Player.findById(req.params.id).populate("soldTo", "teamName logo color");
        if (!player) {
...
router.patch("/:id/pool", auth_middleware_js_1.authenticateJWT, (0, auth_middleware_js_1.requireRole)(["admin"]), async (req, res) => {
    try {
        ...
        if (action === "add") {
            ...
            const maxOrderPlayer = await Player_js_1.Player.findOne({ status: "pool" }).sort({ auctionPoolOrder: -1 });
...
router.put("/pool/order", auth_middleware_js_1.authenticateJWT, (0, auth_middleware_js_1.requireRole)(["admin"]), async (req, res) => {
    try {
        const { ids } = req.body;
        ...
        if (bulkOps.length > 0) {
            await Player_js_1.Player.bulkWrite(bulkOps);
        }
